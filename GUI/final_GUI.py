...
class Settings(customtkinter.CTkToplevel):
    ...
    def setup_options(self, buttons:list = [], names:list = []):
        ...
        for i in range (len(buttons)):
            if (i < 2):
                if (i % 2 == 0):
                    buttons[i].configure(text=names[i])
    ...
    def build_menu(self):
        ...
        global SPEED
        print(self.theme)
        self.splash = customtkinter.CTkLabel(self, text="Settings", text_color=("white"), bg_color="white", anchor='center', font=("Yu Mincho Demibold", 85))
        self.splash.grid(column=3, columnspan=3, row=0, sticky="news")

        self.depth_ = customtkinter.CTkLabel(self, font=("Trebuchet MS", 30), fg_color="white")
    ...
    def depth(self, n):
        ...
        # print(n)
        DEPTH = n
        self.depth_.configure(text=f"Depth:{n}")
        self.depth_.update()
        print(DEPTH)
