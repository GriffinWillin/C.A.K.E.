...
class MainMenu(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.splash.grid(column=2, columnspan=4, row=0, rowspan=2)

        self.play = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", bg_color="transparent", text="+", font=("Trebuchet MS", 30), anchor="center", command=master.add)
        self.play.grid(column=3, columnspan=2, row=3, sticky="news")

        self.characters = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", text="-", font=("Trebuchet MS", 30), anchor="center", command=master.remove)
        self.characters.grid(column=3, columnspan=2, row=5, sticky="news")
        
        self.options = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", text="OPTIONS", font=("Trebuchet MS", 30), anchor="center", command=master.open_settings)
        self.options.grid(column=3, columnspan=2, row=7, sticky="news")
...
class Settings(customtkinter.CTkToplevel):
    def __init__(self, master):
        ...
        self.ARENAS = ["Ocean", "Forest"]
        self.OPTIONS = {self.fsb:"FULLSCREEN", self.raytrace:"TOGGLE RAY-TRACING", self.arena_toggle:f"Current Arena: {self.ARENAS[self.bg]}", self.mute:f"{mute}", self.controls:"KEYBINDS", self.credits:"CREDITS"}
    ...
    def build_menu(self):
        ...
        self.cut_depth = customtkinter.CTkButton(self, command=self.fs)
        self.cut_speed = customtkinter.CTkButton(self, command=self.ask)
        # self.other = customtkinter.CTkButton(self, command=self.credit)
        self.controls = customtkinter.CTkButton(self, command=self.set_binds)
        if (music):
    ...
    def set_binds(self): # turn this into a help icon?
        list = BindKeys(self)
