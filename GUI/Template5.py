...
class MainMenu(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.splash = customtkinter.CTkLabel(master, text="C.A.K.E", text_color="#E57373", bg_color="#90CAF9", anchor='center', fg_color="transparent", font=("Yu Mincho Demibold", 140))
        # self.splash.place(x=self.winfo_screenwidth()//7, y=75, anchor="center")
        # self.splash.grid(column=0, columnspan=6, row=0, rowspan=1)

        self.diagram = customtkinter.CTkLabel(master, image=images[1], text="", anchor='center')
        self.diagram.grid(column=1, columnspan=8, row=3, rowspan=4)

        self.add_slice = customtkinter.CTkButton(master, text_color="white", fg_color="#7986CB", bg_color="transparent", text="+", font=("Trebuchet MS", 100), anchor="center", command=exit)
        self.add_slice.grid(column=2, columnspan=2, row=19, rowspan=6, sticky="news")
        
        self.remove_slice = customtkinter.CTkButton(master, text_color="white", fg_color="#7986CB", bg_color="transparent", text="-", font=("Trebuchet MS", 100), anchor="center", command=exit)
        self.remove_slice.grid(column=5, columnspan=2, row=19, rowspan=6, sticky="news")
        ...
        self.slice = customtkinter.CTkLabel(master, text=f"1", font=("Trebuchet MS", 85), anchor="center", text_color="white")
        self.slice.grid(column=2, columnspan=5, row=13, sticky="news")

        self.themes = customtkinter.CTkButton(master, text_color="white", fg_color="#7986CB", text="INFO", font=("Trebuchet MS", 80), anchor="center", command=exit)
        self.themes.grid(column=9, columnspan=8, row=1, rowspan=6, sticky="news", ipady = 100)
        
        self.options = customtkinter.CTkButton(master, text_color="white", fg_color="#7986CB", text="THEMES", font=("Trebuchet MS", 80), anchor="center", command=exit)
        self.options.grid(column=18, columnspan=7, row=1, rowspan=6, sticky="news", ipady = 100)

        self.start = customtkinter.CTkButton(master, text_color="white", fg_color="#66BB6A", text=" START", font=("Trebuchet MS", 80), anchor="center", command=exit)
        self.start.grid(column=9, columnspan=8, row=9, rowspan=16, sticky="news", ipady=110)

        self.leave = customtkinter.CTkButton(master, text_color="white", fg_color="#E57373", text=" EXIT", font=("Trebuchet MS", 80), anchor="center", command=exit)
        self.leave.grid(column=18, columnspan=7, row=9, rowspan=16, sticky="news", ipady = 110)
