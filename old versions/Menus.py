...
class MainMenu(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.splash = customtkinter.CTkLabel(master, text="C.A.K.E.", text_color="white", bg_color="#90CAF9", anchor='center', fg_color="transparent", font=("Yu Mincho Demibold", 100))
        self.splash.grid(column=0, columnspan=4, row=0, rowspan=2)

        self.diagram = customtkinter.CTkLabel(master, image=master.DIAGRAMS[0], text_color="white", bg_color="#90CAF9", anchor='center', fg_color="transparent", font=("Yu Mincho Demibold", 100))
        self.diagram.grid(column=2, columnspan=4, row=3, rowspan=2)

        self.add_slice = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", bg_color="transparent", text="+", font=("Trebuchet MS", 30), anchor="center", command=master.add_slices)
        self.add_slice.grid(column=3, columnspan=2, row=3, sticky="news")
        
        self.remove_slice = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", bg_color="transparent", text="+", font=("Trebuchet MS", 30), anchor="center", command=master.remove_slices)
        self.remove_slice.grid(column=3, columnspan=2, row=3, sticky="news")
        
        self.slice_num = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", bg_color="transparent", text="+", font=("Trebuchet MS", 30), anchor="center", command=master.remove_slices)
        self.slice_num.grid(column=3, columnspan=2, row=3, sticky="news")

        self.themes = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", text="select a theme", font=("Trebuchet MS", 30), anchor="center", command=master.themes)
        self.themes.grid(column=3, columnspan=2, row=5, sticky="news")
        
        self.options = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", text="info", font=("Trebuchet MS", 30), anchor="center", command=master.open_settings)
        self.options.grid(column=6, columnspan=2, row=0, sticky="news")

        self.leave = customtkinter.CTkButton(master, text_color="white", fg_color="#655482", text="EXIT", font=("Trebuchet MS", 30), anchor="center", command=exit)
        self.leave.grid(column=3, columnspan=2, row=9, sticky="news")
...
class App(customtkinter.CTk):
    ...
    def get_background(self, image:str):
        image = PIL.Image.open(os.path.join("Backgrounds", f"{image}"))
...
class CharacterSelect(customtkinter.CTkToplevel): # turn this into a theme select
    ...
    def setup_selection(self, buttons:list = [], images:list = [], names:list = []):
        ...
        for i in range(len(buttons)):
            image = PIL.Image.open(os.path.join("Characters", f"{images[i]}.png"))
    ...
    def select(self, name):
        global character
        character = (os.path.join("Characters", f"{name}.png"))
